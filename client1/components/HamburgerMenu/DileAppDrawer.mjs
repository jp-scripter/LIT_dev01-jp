// import { html, css, LitElement } from "lit";
   ...
   import { DileCloseOnEscPressed } from './dile-close-on-esc-pressed-mixin.mjs';
  ...
  static get styles() {
    ...
    return css`
      ...
      .menu {
        ...
        transition: transform 0s;  /* was 0.3s ease; */
        overflow: auto;
      }

      section {
        transform: translateX(-100vw);
        transition: transform 0s; /* was 0.2s ease 0.3s; */ 
      }
...
// ---------------------------------------------------------------------------
  ...
  _documentClose() {
        if (this.opened) {
            this.close();
            this.dispatchEvent( new CustomEvent( DILE_APP_DRAWER_EVENT_CLOSE_OUTSIDE, { bubbles: true, composed: true, } ) );
            }
        } // eof _documentClose()  
// ---------------------------------------------------------------------------
  ...
  close( ) { this.opened = false;
             this.dispatchEvent( new CustomEvent( DILE_APP_DRAWER_EVENT_CLOSED,       { bubbles: true, composed: true } ) );
             }
// ---------------------------------------------------------------------------

  _contentClick( e ) { e.stopPropagation(); }
